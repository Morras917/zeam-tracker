...
import { supabase, Merchant } from '../lib/supabaseClient'
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          {[{ href: '/dashboard', l: 'Dashboard' }, { href: '/partnerships', l: 'Partnerships' }, { href: '/merchants', l: 'Merchants' }, { href: '/business-accounts', l: 'Business Accounts' }].map(link => (
            <Link key={link.href} href={link.href} style={{ color: link.href === '/merchants' ? 'white' : '#a0a0c0', textDecoration: 'none', fontSize: '0.875rem' }}>{link.l}</Link>
          ))}
        </div>
      </div>
...
const emptyForm = { name: '', location: '', contact_name: '', contact_email: '', contact_phone: '', status: 'Pipeline' as Merchant['status'], notes: '' }
...
export default function Merchants() {
  ...
  const openEdit = (item: Merchant) => {
    setEditing(item)
    setForm({ name: item.name, location: item.location || '', contact_name: item.contact_name || '', contact_email: item.contact_email || '', contact_phone: item.contact_phone || '', status: item.status, notes: item.notes || '' })
    setShowModal(true)
  }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    if (editing) {
      await supabase.from('merchants').update({ ...form, updated_at: new Date().toISOString() }).eq('id', editing.id)
    } else {
      await supabase.from('merchants').insert([form])
    }
    await fetchData()
    setShowModal(false)
    setSaving(false)
  ...
  const filtered = items.filter(item =>
    ...
    (item.name.toLowerCase().includes(search.toLowerCase()) || (item.location || '').toLowerCase().includes(search.toLowerCase()))
  )
  ...
  return (
    <>
      <Head><title>Merchants — Zeam Tracker</title></Head>
      <NavBar onLogout={handleLogout} />
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            <h1 style={{ margin: '0 0 0.25rem', fontSize: '1.75rem', fontWeight: '800' }}>Merchants</h1>
            <p style={{ margin: 0, color: '#a0a0c0' }}>{items.length} total merchants</p>
          </div>
          <button className="btn-primary" onClick={openAdd}>+ Add Merchant</button>
        </div>

        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search merchants..." style={{ maxWidth: '280px' }} />
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {['All', ...STATUSES].map(s => (
              <button key={s} onClick={() => setFilterStatus(s)} style={{ padding: '0.375rem 0.75rem', borderRadius: '6px', border: '1px solid', borderColor: filterStatus === s ? '#f5c542' : '#2a2a45', background: filterStatus === s ? '#f5c54222' : 'transparent', color: filterStatus === s ? '#f5c542' : '#a0a0c0', cursor: 'pointer', fontSize: '0.8rem' }}>{s}</button>
            ))}
          </div>
        </div>
              ...
              <tbody>
                {filtered.map(item => (
                  <tr key={item.id}>
                    ...
                    <td style={{ color: '#a0a0c0' }}>{item.location || '—'}</td>
                ...
                {filtered.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center', color: '#a0a0c0', padding: '2rem' }}>No merchants found</td></tr>}
              </tbody>
            </table>
          )}
        </div>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal-content">
            <h2 style={{ margin: '0 0 1.5rem', color: 'white' }}>{editing ? 'Edit Merchant' : 'Add Merchant'}</h2>
            <form onSubmit={handleSave}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                <div><label style={{ display: 'block', color: '#a0a0c0', fontSize: '0.8rem', marginBottom: '0.4rem' }}>Name *</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required /></div>
                <div><label style={{ display: 'block', color: '#a0a0c0', fontSize: '0.8rem', marginBottom: '0.4rem' }}>Location</label><input value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} /></div>
                ...
                <div><label style={{ display: 'block', color: '#a0a0c0', fontSize: '0.8rem', marginBottom: '0.4rem' }}>Status</label><select value={form.status} onChange={e => setForm({ ...form, status: e.target.value as Merchant['status'] })}>{STATUSES.map(s => <option key={s}>{s}</option>)}</select></div>
